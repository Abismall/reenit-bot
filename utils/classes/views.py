...
class ScrimLobby(View):
    def __init__(self, title, host):
        ...
        self.servers = requests.get(
            f"http://{API_BASE_URL}:{API_PORT}/dathost/available/").json()
        self.locations_in_use = []
        check_for_locations = requests.get(
            f"http://{API_BASE_URL}:{API_PORT}/reenit/scrims/")
    ...
    def teams_selection_window(self):
        self.clear_items()
        self.add_item(TeamSelectionButton(
            "Join team one", self.current_title, 1))
        self.add_item(TeamSelectionButton(
            "Join team two", self.current_title, 2))
        self.add_item(TeamSelectionVote("vote for teams", self.host))
        self.add_item(TeamSelectionShuffle(
            "Shuffle", self.current_title, self.host))
    ...
    def get_server_details(self):
        server_details = requests.get(
            f"http://{API_BASE_URL}:{API_PORT}/dathost/details/?server_id={self.server_id}")
    ...
    async def start_server(self):
        requests.post(
            f"http://{API_BASE_URL}:{API_PORT}/dathost/start/?server_id={self.server_id}")
    ...
    def get_data(self):
        ...
        check_for_lobby = requests.get(
            f"http://{API_BASE_URL}:{API_PORT}/reenit/scrim/", json=payload).json()
        try:
            self.team_1 = check_for_lobby["lobby"]["team_one"]
            self.team_2 = check_for_lobby["lobby"]["team_two"]
            self.current_lobby = [player["username"]
                                  for player in check_for_lobby["players"]]
    ...
    def update_lobby(self):
        ...
        lobby = requests.put(
            f"http://{API_BASE_URL}:{API_PORT}/reenit/", json=data)
    ...
    def set_location_and_server_id(self, location):
        ...
        index = [location[0]
                 for location in self.available_locations].index(location)
